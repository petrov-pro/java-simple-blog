...
public class CategoryModel extends blog.system.model.Model implements TreeIntf<Category> {
    ...
    public boolean findByPkFree(int category_id) {
        CategoryImpl ci = (CategoryImpl) DaoFactory.getDao("CategoryImpl");
        try {
            category = ci.findByPkFree(category_id);
        } catch (PersistException p) {
            Logger.write(p.toString());
            return false;
        }
    ...
    public boolean findByAlias(String user_id_s, String category_alias) {
        CategoryImpl ci = (CategoryImpl) DaoFactory.getDao("CategoryImpl");
        try {
            int user_id = Integer.parseInt(user_id_s);
            category = ci.findByAlias(user_id, category_alias);
        } catch (PersistException p) {
            Logger.write(p.toString());
            return false;
        }
    ...
    public void findAllCustom(int page, String seacrh) {
        CategoryImpl i = (CategoryImpl) DaoFactory.getDao("CategoryImpl");
        try {
            categories = i.findAllCustom(page, seacrh);
            count(seacrh);
        } catch (PersistException p) {
            Logger.write(p.toString());
        }
